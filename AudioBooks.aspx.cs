...
namespace ReaderZ_LMS
{
    public partial class AudioBooks : System.Web.UI.Page
    {
        ...
        {
            FillData("select * from tbl_audiobooks ORDER BY ID DESC");
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    FillData("select * from tbl_audiobooks where AudioBook_Name = '" + txtSearch.Text + "' ORDER BY ID DESC");
                }
                else
                {
                    FillData("select * from tbl_audiobooks where AudioBook_Name = '" + txtSearch.Text + "' and Language = '" + ddLangauge.Text + "' ORDER BY ID DESC");
                }
            }
        }
        ...
        protected void ddLangauge_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                FillData("select * from tbl_audiobooks where Language = '" + ddLangauge.Text + "' ORDER BY ID DESC");
            }
        }
